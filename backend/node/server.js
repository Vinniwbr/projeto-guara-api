...
app.get("/chats", async (req, res) => {
	try {
		const resultado = await pool.query(`
      SELECT
        c.id, c.nome, c.tipo, c.criado_em,
...
// Aceita o token JWT de 3 formas:
//   1) Query string ?token=... (vindo do frontend via redirect de página)
//   2) Header Authorization: Bearer ... (vindo de chamadas fetch)
//   3) Query string ?usuario_id=... (fallback de desenvolvimento)
app.get("/auth/google", async (req, res) => {
	let usuarioId = null;

	// 1) Token via query string (usado pelo frontend ao redirecionar o browser)
	if (req.query.token) {
		try {
	...
	// 2) Token via header Authorization (usado por chamadas fetch/Postman)
	...
	// 3) Fallback: usuario_id direto na query (desenvolvimento/testes)
